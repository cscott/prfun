...
module.exports = function(Promise) {
  ...
  // Like Promise.race, but return first `n` which fulfil.
  Promise.prototype.some = function(n) {
    var P = this.constructor || Promise;
    return this.then(function(value) {
      return P.some(value, n);
    });
  };

  var arrayForEach = Array.prototype.forEach;
  Promise.some = function(pArray, n) {
    var P = this || Promise;
    var result = [];
    return P.resolve(pArray).then(function(arr) {
      return new P(function(resolve, reject) {
        forEach.call(arr, function(pElem, index, arr_) {
          P.resolve(pElem).then(function(elem) {
            if (result.length < n) {
              result.push(elem);
              if (result.length >= n) {
                resolve(result);
