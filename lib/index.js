...
module.exports = function(Promise) {
  ...
  // This is often needed for Monads; it's like resolve but it doesn't
  // add an extra layer of casting.
  Promise._cast = function(v) {
    return this.resolve(v).then(function(v) { return v; });
  };
  // Need to reimplement Promise.all in terms of _cast :(
  ...
  Promise.all = function(iterable) {
    ...
    try {
      ...
      for (var index = 0; index < itemLen; index++) {
        ...
        var nextPromise = C._cast(next); // this is the only change
  ...
  // Need to reimplement Promise.race in terms of _cast :(
  ...
  Promise.map = function(pArray, callback, thisArg) {
    var P = this || Promise;
    return P._cast(pArray).then(function(arr) {
      return P.all(arrayMap.call(arr, function(pElem, index, arr_) {
        return P._cast(pElem).then(function(elem) {
          var t = (thisArg === undefined) ? this : thisArg;
          return P.resolve(callback.call(t, elem, index, arr_));
        });
      }));
    });
  };
  ...
  Promise.props = function(pObj) {
    var P = this || Promise;
    return P._cast(pObj).then(function(obj) {
      var result = {};
  ...
  var reducer = function(P, callback) {
    return function(pPrev, pCurrent, index, arr) {
      return P._cast(pPrev).then(function(prev) {
        return P._cast(pCurrent).then(function(current) {
  ...
  Promise.reduce = function(pArray, callback) {
    ...
    if (arguments.length <= 2) {
      ...
      var initialValue = P._cast(arguments[2]);
      return P._cast(pArray).then(function(arr) {
  ...
  Promise.prototype.call = function(propertyName) {
    ...
    return this.then(function(obj) {
      return P.resolve(obj[propertyName].apply(obj, args));
    });
  };
  ...
  Promise.prototype.get = function(propertyName) {
    var P = this.constructor || Promise;
    return this.then(function(obj) {
      return P.resolve(obj[propertyName]);
    });
  };

  Promise.prototype['return'] = function(v) {
    var P = this.constructor || Promise;
    return this.then(function() { return P.resolve(v); });
  };
  ...
  Promise.delay = function(pValue, ms) {
    ...
    return P._cast(pValue).then(function(value) {
  ...
  // ---------- try/caught/finally --------------
  // XXX DIDN'T AUDIT BELOW THIS POINT FOR resolve WHICH SHOULD BE _cast
  ...
  Promise.prototype.bind = function(newThis) {
    var SuperPromise = this._superPromise || this.constructor || Promise;
    ...
    BoundPromise.prototype._superPromise = SuperPromise;
    ...
    return newThis ? BoundPromise._cast(this) : SuperPromise._cast(this);
  };

  // Generators.
  // Implementation borrowed from Q.async()
  Promise.fromGenerator = function(makeGenerator) {
    var P = this || Promise;
    return function() {
  ...
  // Monadic promises.  Protect the content of the promise; every
  // call to `.resolve` adds exactly one layer of wrapping, and each
  // call to `.then` removes exactly one.
  var Monadic = Promise.Monadic = function(exec) {
      Promise.call(this, function(f, r) {
          exec(function(v) { f([v]); }, r); // wrap v
      });
  };
  Object.setPrototypeOf(Monadic, Promise);
  Monadic.prototype = Object.create(Promise.prototype);
  Monadic.prototype.constructor = Monadic;

  Monadic.prototype.then = (function(superThen) {
    return function(f, r) {
      var ff = function(v) { return f ? f(v[0]) : v[0]; }; // unwrap v
      return superThen.call(this, ff, r);
    };
  })(Monadic.prototype.then);

  // XXX Note that I wouldn't have to override `resolve` if it weren't for
  // the optimization in step 2 of Promise.resolve (formerly Promise.cast).
  Monadic.resolve = function(v) {
    return new Monadic(function(r) { r(v); });
  };
